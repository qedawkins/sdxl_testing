...
#img_map = affine_map<(b, ooc, ih, iw, ioc, oic, kh, kw, iic) -> (b, oic, ih + kh, iw + kw, iic)>
#filter_map = affine_map<(b, ooc, ih, iw, ioc, oic, kh, kw, iic) -> (ooc, oic, kh, kw, ioc, iic)>
#out_map = affine_map<(b, ooc, ih, iw, ioc, oic, kh, kw, iic) -> (b, ooc, ih, iw, ioc)>
...
//===----------------------------------------------------------------------===//
  ...
  transform.named_sequence @match_tiled_conv2d(%conv: !transform.any_op {transform.readonly})
    ...
    ^bb0(%arg0: !tiled_image_type, %arg1: !tiled_filter_type, %1: !tiled_acc_type):
      %2 = linalg.generic {indexing_maps = [#img_map, #filter_map, #out_map], iterator_types = ["parallel", "parallel", "parallel", "parallel", "parallel", "reduction", "reduction", "reduction", "reduction"]} ins(%arg0, %arg1 : !tiled_image_type, !tiled_filter_type) outs(%1 : !tiled_acc_type) {
      ^bb0(%in: f16, %in_0: f16, %out: f32):
        %5 = arith.extf %in : f16 to f32
        %6 = arith.extf %in_0 : f16 to f32
        %7 = arith.mulf %5, %6 : f32
