...
#map2 = affine_map<(b, ooc, oic, ih, iw, ioc, kh, kw, iic) -> (b, oic, ih + kh, iw + kw, iic)>
#map3 = affine_map<(b, ooc, oic, ih, iw, ioc, kh, kw, iic) -> (ooc, oic, kh, kw, ioc, iic)>
#map4 = affine_map<(b, ooc, oic, ih, iw, ioc, kh, kw, iic) -> (b, ooc, ih, iw, ioc)>
module {
  func.func @forward(%arg0: !image_type, %arg1: !filter_type) -> !result_type {
    ...
    %9 = flow.dispatch.region -> (!result_type) {
      ...
      %2 = linalg.generic {indexing_maps = [#map2, #map3, #map4], iterator_types = ["parallel", "parallel", "reduction", "parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%arg0, %arg1 : !image_type, !filter_type) outs(%1 : !acc_type) {
